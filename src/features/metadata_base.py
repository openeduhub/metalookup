...
from lib.settings import USE_LOCAL_IF_POSSIBLE, USE_RE2
...
class MetadataBase:
    ...
    def _prepare_start(self, key: str) -> tuple[float, WebsiteData]:
        self._logger.debug(f"Starting {self.__class__.__name__} {key}.")
        before = get_utc_now()
    ...
    def setup(self) -> None:
        ...
        after = get_utc_now()
        self._logger.debug(
            f"{self.__class__.__name__}: Setup done at {get_utc_now()}, took {after - before}s."
        )
